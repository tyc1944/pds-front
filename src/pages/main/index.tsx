...
class Main extends React.Component<Props> {
  ...
  componentDidMount() {
    ...
    main
      ...
      .then(res => this.initChart(_.reverse(res.data)));
    main.getStatisticsWholeCount().then(res =>
      this.setState({
        caseWholeCount: res.data
      })
    );
    main.getStatisticsTodoList().then(res =>
      this.setState({
        todoList: res.data
      })
    );
  ...
  initChart = (data: []) => {
    const columnPlot = new GroupedColumn(
      document.getElementById("barChartId") as HTMLElement,
      {
        title: {
          visible: true,
          text: "本院近一年线索案件量统计"
  ...
  onTodoItemClick = (todo: Todo) => {
    ...
    if (todo.todoCategory === "new_clue") {
      history.push(
        `/index/clue/${role}/judge/${todo.todoStatus}/${todo.todoId}`
      );
    } else {
      let tabIndex = "1";
      ...
      history.push(
        `/index/supervise/${role}/${todo.todoStatus}/${tabIndex}/${todo.todoId}`
      );
    }
  };
  ...
  render() {
    ...
    return (
      ...
      >
        {this.state.showSuperviseAlertModal && (
          <SuperviseAlertModal
            onDetailClick={todo => {
            ...
            visible={this.state.showSuperviseAlertModal}
          ></SuperviseAlertModal>
        )}
        ...
        <BoxContainer>
          <BoxContainerInner flex={0.7}>
            ...
            >
              <div
                style={{ width: "calc(100% - 492px - 19px)" }}
                id="barChartId"
              ></div>
              <div
                style={{
                ...
                <div className="global-info-panel">
                  ...
                  {main.userProfile.role === "NORMAL_USER" && (
                    <div className="global-info-category">
                      ...
                      <div>
                        <div>
                          {caseWholeCount.pendingProcessInvestigationCount}
                        </div>
                        <div>侦查监督</div>
                      </div>
                      <div>
                      ...
                      <div>
                        <div>
                          {caseWholeCount.pendingProcessAdministrationCount}
                        </div>
                        <div>行政监督</div>
                      </div>
                    </div>
                  ...
                  <div
                    className={`global-info-message ${
                      this.state.todoList.length === 0 ? "empty" : ""
                    }`}
                  >
                    {this.state.todoList.map((item, index) => (
                      <div key={index}>
                        ...
                        <div style={{ color: "#A6A6A6" }}>
                          {formatTimeYMDHMS(item.createdTime)}
                        </div>
                      </div>
                    ))}
                    {this.state.todoList.length === 0 && (
                      <div className="global-info-message-empty">
                        没有待处理事项
                      </div>
                    )}
                  </div>
