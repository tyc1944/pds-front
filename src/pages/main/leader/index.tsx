...
class LeaderMainDataList extends React.Component<MainDataListProps> {
    ...
    render() {
        ...
        return <Tabs defaultActiveKey="1" onChange={this.onTabChange}>
            <TabPane tab={<TableNameWithNumber name="待审批线索" count={caseWholeCount.pendingExamineClueCount} />} key="1">
                <PendingExamineTable activeIndex={this.state.activeIndex} onDetailClick={this.onDetailClick}></PendingExamineTable>
            </TabPane>
            <TabPane tab={<TableNameWithNumber name="待审批侦察监督" count={caseWholeCount.pendingExamineInvestigationCount} />} key="2">
                <InvestigationTable activeIndex={this.state.activeIndex} onDetailClick={this.onDetailClick} />
            </TabPane>
            <TabPane tab={<TableNameWithNumber name="待审批审判监督" count={caseWholeCount.pendingExamineTrialCount} />} key="3">
                <TrialTable activeIndex={this.state.activeIndex} onDetailClick={this.onDetailClick} ></TrialTable>
            </TabPane>
            <TabPane tab={<TableNameWithNumber name="待审批执行监督" count={caseWholeCount.pendingExamineExecutionCount} />} key="4">
                <ExecutionTable activeIndex={this.state.activeIndex} onDetailClick={this.onDetailClick} ></ExecutionTable>
            </TabPane>
            <TabPane tab={<TableNameWithNumber name="待审批行政监督" count={caseWholeCount.pendingExamineAdministrationCount} />} key="5">
