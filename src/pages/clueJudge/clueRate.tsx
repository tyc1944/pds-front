...
const ClueRateStar = ({ rate = 0 }: { rate?: number }) => (
  <>
    {(() => {
      let tmp = [];
      for (let i = 0; i < 5; i++) {
        if (i < rate) {
          ...
          tmp.push(<StarFilled style={{}} translate="true" key={i} />);
        }
      }
      return tmp;
    })()}
...
const personalRiskOptions = [{ label: "涉案人有自杀、自残等状况", value: 0 }];
...
export const ClueRate = (props: {
  ...
  const [publicOpinionRisk, setPublicOpinionRisk] = React.useState(
    [] as number[]
  );
  ...
  const submitable =
    ...
    amountInvolved !== -1 ||
    clueSource.length !== 0 ||
    personalRisk.length !== 0 ||
    status.length !== 0 ||
    publicOpinionRisk.length !== 0 ||
    petitionRisk.length !== 0;
    ...
    <div className="clue-rate">
      <div className="clue-rate-category">
        ...
        <div className="clue-rate-row">
          ...
          <div>
            <Radio.Group
              onChange={e => setReportWay(e.target.value)}
              value={reportWay}
            >
        ...
        <div className="clue-rate-row">
          ...
          <div>
            <Radio.Group
              onChange={e => setInfringer(e.target.value)}
              value={infringer}
            >
          ...
          <div>
            <Radio.Group
              onChange={e => setInfringementAct(e.target.value)}
              value={infringementAct}
            >
              {infringerOptions.map(item => (
                <Radio value={item.index} key={item.index}>
                  {item.name}
        ...
        <div className="clue-rate-row">
          ...
          <div>
            <Checkbox.Group
              ...
              onChange={vals => setPersonalRisk(vals as number[])}
            />
          </div>
        </div>
        <div className="clue-rate-row">
          ...
          <div>
            <Checkbox.Group
              ...
              onChange={vals => setStatus(vals as number[])}
            />
          </div>
        </div>
        <div className="clue-rate-row">
          ...
          <div>
            <Checkbox.Group
              ...
              options={publicOpinionRiskOptions}
              onChange={vals => setPublicOpinionRisk(vals as number[])}
            />
          </div>
        </div>
        <div className="clue-rate-row">
          ...
          <div>
            <Checkbox.Group
              value={petitionRisk}
              options={petitionRiskOptions}
              onChange={vals => setPetitionRisk(vals as number[])}
            />
          </div>
        </div>
      </div>
            ...
            {submitable && (
              <>
                <div>
                  ...
                  {reportWay !== -1 &&
                    `举报方式${
                      reportWayOptions.filter(
                        item => item.index === reportWay
                      )[0].name
                    }；`}
                  {reportContent !== -1 &&
                    `举报材料${
                      reportContentOptions.filter(
                        item => item.index === reportContent
                      )[0].name
                    }；`}
                  {infringer !== -1 &&
                    `侵权人${
                      infringerOptions.filter(
                        item => item.index === infringer
                      )[0].name
                    }；`}
                  {infringementAct !== -1 &&
                    `侵权行为${
                      infringerOptions.filter(
                        item => item.index === infringementAct
                      )[0].name
                    }；`}
                  {amountInvolved !== -1 &&
                    `涉案金额${
                      amountInvolvedOptions.filter(
                        item => item.index === amountInvolved
                      )[0].name
                    }；`}
                  {clueSource.length !== 0 &&
                    `线索来源${clueSourceOptions
                      .filter(item => clueSource.indexOf(item.value) !== -1)
                      .map(item => item.label)
                      .join("，")}；`}
                  {personalRisk.length !== 0 &&
                    `人身风险${personalRiskOptions
                      .filter(item => personalRisk.indexOf(item.value) !== -1)
                      .map(item => item.label)
                      .join("，")}；`}
                  {status.length !== 0 &&
                    `身份情况${statusOptions
                      .filter(item => status.indexOf(item.value) !== -1)
                      .map(item => item.label)
                      .join("，")}；`}
                  {publicOpinionRisk.length !== 0 &&
                    `舆情风险${publicOpinionRiskOptions
                      .filter(
                        item => publicOpinionRisk.indexOf(item.value) !== -1
                      )
                      .map(item => item.label)
                      .join("，")}；`}
                  {petitionRisk.length !== 0 &&
                    `上访风险${petitionRiskOptions
                      .filter(item => petitionRisk.indexOf(item.value) !== -1)
                      .map(item => item.label)
                      .join("，")}；`}
                </div>
                <div
  ...
  return (
    ...
    >
      <div className="clue-rate-info">
        <div>
          线索评级：<span>{props.rate ? props.rate : "--"}</span>
        </div>
        <div>
      ...
      >
        ...
        <div>
          ...
          <div>
            {reportWayOptions
              .filter(item => item.index === rateJson.reportWay)
              .map(item => item.name)
              .join(",")}
          </div>
          <div>举报材料：</div>
          <div>
            {reportContentOptions
              .filter(item => item.index === rateJson.reportContent)
              .map(item => item.name)
              .join(",")}
          </div>
        </div>
        <div>
          ...
          <div>
            {infringerOptions
              .filter(item => item.index === rateJson.infringer)
              .map(item => item.name)
              .join(",")}
          </div>
          <div>侵权行为：</div>
          <div>
            {infringerOptions
              .filter(item => item.index === rateJson.infringementAct)
              .map(item => item.name)
              .join(",")}
          </div>
        </div>
        <div>
          <div>涉案金额：</div>
          <div>
            {amountInvolvedOptions
              .filter(item => item.index === rateJson.amountInvolved)
              .map(item => item.name)
              .join(",")}
          </div>
        </div>
      ...
      <div className="clue-rate-modal">
        ...
        <div>
          ...
          <div>
            {clueSourceOptions
              .filter(item => rateJson.clueSource.indexOf(item.value) !== -1)
              .map(item => item.label)
              .join("，")}
          </div>
          <div>人身风险：</div>
          <div>
            {personalRiskOptions
              .filter(item => rateJson.personalRisk.indexOf(item.value) !== -1)
              .map(item => item.label)
              .join("，")}
          </div>
        </div>
        <div>
          ...
          <div>
            {statusOptions
              .filter(item => rateJson.status.indexOf(item.value) !== -1)
              .map(item => item.label)
              .join("，")}
          </div>
        </div>
        <div>
          <div>舆情风险：</div>
          <div>
            {publicOpinionRiskOptions
              .filter(
                item => rateJson.publicOpinionRisk.indexOf(item.value) !== -1
              )
              .map(item => item.label)
              .join("，")}
          </div>
        ...
        <div>
          ...
          <div>
            {petitionRiskOptions
              .filter(item => rateJson.petitionRisk.indexOf(item.value) !== -1)
              .map(item => item.label)
              .join("，")}
          </div>
        </div>
...
export const ClueRateInfo = inject("clue")((props: { clue?: ClueStore }) => {
  ...
  return (
    <>
      ...
      <div className="clue-rate-info">
        <div>
          线索评级：
          <span>{clueProcessData.rate ? clueProcessData.rate : "--"}</span>
        </div>
        <div>
